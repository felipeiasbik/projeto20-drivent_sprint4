...
describe('GET /booking', () => {
  ...
  describe('when token is valid', () => {
    it('should respond with status 200 and booking when user has a booking', async () => {
      ...
      const booking = await createBookingFactory(user.id, room.id);
...
describe('PUT /booking', () => {
  ...
  describe('when token is valid', () => {
    it('should respond with status 200 and return new booking id', async () => {
      ...
      const room = await createRoom(hotel.id);
      const booking = await createBookingFactory(user.id, room.id);
      const editBooking = await createRoom(hotel.id);
